...
export default function Segmentos() {
  ...
  function createDataSegmentos(CodSegmen, DesSegmen) {
    return { CodSegmen, DesSegmen };
  }

  let rowsSegmentos = [];
  rowsSegmentos = dados.map(each =>
    createDataSegmentos(each.CodSegmen, each.DesSegmen)
  );

  const aplicaFiltro = valor => {
    if (valor.length >= 1) setFiltro("DesSegmen LIKE '%" + valor + "%'");
    else setFiltro(null);
  };

  const montaLink = codigo => {
    history.push(`/Segmentos/CadSegmentos/CodSegmen=${codigo}`);
  };

  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, rowsSegmentos.length - page * rowsPerPage);
  ...
  useEffect(() => {
    ...
    const runEffect = async () => {
      try {
        await buscaSegmentos(filtro, undefined).then(data => {
  ...
  const excluiRegistroSegmento = CodSegmen => {
    ...
    const exclui = async () => {
      try {        
        if ((await excluiSegmento(CodSegmen)) === false) {
          setMensagem({
            titulo: 'Erro',
            conteudo:
  ...
  return (
    <Fragment>
      ...
      <PageTitle titleHeading="Segmentos" titleDescription="Consulta" />
                ...
                {(rowsPerPage > 0
                  ...
                  <TableRow key={row.CodSegmen}>                     
                    ...
                    <TableCell align="right" size="small">
                      ...
                      <IconButton
                        aria-label="delete"
                        onClick={() => clickExcluir(row.CodSegmen)}>
                        <Delete />
                      </IconButton>
                    </TableCell>
                  </TableRow>
